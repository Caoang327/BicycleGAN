...
def define_G(input_nc, output_nc, nz, ngf,
    ...
    if which_model_netG == 'unet_128' and where_add == 'input':
        ...
        netG = G_Unet_add_input(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                                use_dropout=use_dropout, gpu_ids=gpu_ids, upsample=upsample)
    elif which_model_netG == 'res_g' and where_add == 'input':
        netG = GlobalGenerator(input_nc, output_nc, nz, ngf=64, n_downsampling=3, n_blocks=6, norm_layer=norm_layer,
...
class GlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, nz, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
                 padding_type='reflect'):
        assert (n_blocks >= 0)
        super(GlobalGenerator, self).__init__()
        activation = nn.ReLU(True)
        self.nz = nz
        model = [nn.ReflectionPad2d(3), nn.Conv2d(input_nc+nz, ngf, kernel_size=7, padding=0), norm_layer(ngf), activation]
        ### downsample
